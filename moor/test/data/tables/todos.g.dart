...
class TableWithoutPKCompanion extends UpdateCompanion<CustomRowClass> {
  ...
  TableWithoutPKCompanion setTableWithoutPKCompanion(CustomRowClass from) {
    return TableWithoutPKCompanion(
      notReallyAnId: Value(from.notReallyAnId),
      someFloat: Value(from.someFloat),
      custom: Value(from.custom),
    );
  }

  factory TableWithoutPKCompanion.fromRowClass(CustomRowClass object,
      {bool nullToAbsent = false}) {
    return TableWithoutPKCompanion(
      custom: Value(object.custom),
    );
  }
  static Map<String, Expression> fromRowClassToColumns(CustomRowClass object,
      {bool nullToAbsent = false}) {
    return TableWithoutPKCompanion.fromRowClass(object,
